class Product{
  ...
  factory Product.fromJson(Map<String, dynamic> json){
    try {
      ...
      print("_getPrice(json['proStrPrice']${_getPrice(json['proStrPrice'] ?? "0.0")}");

      return Product(
        id: json['id'] ?? 0,
        name: json['name'] ?? "",
        description: json['description'] ?? "",
        proPrice: _getPrice(json['price'] ?? "0.0"),

        proStrPrice: json['price'] ?? "0.0",
        image: json['image'] ?? "",
  ...
  static double _getPrice(String strPrice){
    try {
      if (strPrice== null) return 0.0;
